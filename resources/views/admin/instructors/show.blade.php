...
@section('content')
    ...
    <div class="container mx-auto px-8 py-6">
        <div class="bg-white p-6 rounded-lg shadow-md">
            <h2 class="text-xl font-bold text-gray-800 mb-4">Instructor Details</h2>

            <div class="space-y-4">
                <!-- Name -->
                <p><span class="font-semibold text-gray-700">Name:</span> {{ $instructor->name }}</p>

                <!-- Email -->
                <p><span class="font-semibold text-gray-700">Email:</span> {{ $instructor->email }}</p>

                <!-- Phone Number -->
                <p><span class="font-semibold text-gray-700">Phone Number:</span> {{ $instructor->phone_number }}</p>

                <!-- Course -->
                <p><span class="font-semibold text-gray-700">Course:</span> {{ $instructor->course->title ?? 'N/A' }}</p>
                ...
                <div class="bg-white shadow-lg rounded-lg overflow-hidden">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-gray-700 mb-4">Instructor Photo</h3>
                        @if ($instructor->photo)
                            <div class="mb-4">
                                <img src="{{ Storage::url($instructor->photo) }}" alt="{{ $instructor->title }} photo"
                                    class="w-full h-64 object-cover rounded-lg shadow-md">
                            </div>
                            <div class="text-center">
                                <a href="{{ Storage::url($instructor->photo) }}" target="_blank"
