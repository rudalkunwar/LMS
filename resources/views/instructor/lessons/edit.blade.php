...
<div class="container mx-auto px-4 py-8">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Edit Lesson</h2>
    ...
    <form action="{{ route('lessons.update', $lesson->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="flex flex-col">
            ...
            <input type="text" id="title" name="title" value="{{ $lesson->title }}" class="mt-1 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
        </div>

        <!-- Lesson Description -->
        <div class="flex flex-col">
            <label for="description" class="font-semibold text-gray-700">Description</label>
            <textarea id="description" name="description" rows="4" class="mt-1 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>{{ $lesson->description }}</textarea>
        </div>
        ...
        <div class="flex flex-col">
            ...
            <input type="url" id="content_url" name="content_url" value="{{ $lesson->content_url }}" class="mt-1 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="URL for video/document (optional)">
        </div>

        <!-- Lesson Order -->
        <div class="flex flex-col">
            <label for="order" class="font-semibold text-gray-700">Lesson Order</label>
            <input type="number" id="order" name="order" value="{{ $lesson->order }}" class="mt-1 p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
